...
namespace Shared
{
    ...
    {
        public const int DATA_BUFFER_SIZE = 8192;
        ...
        public static string[] SplitIntoPackets(byte[] data, out byte[] remainder)
        {
            ...
            int i = 0;
            while (!int.TryParse(strPieces[i], out _))
            {
                // First segment can't be a packetType - misaligned!
                // We skip the first segment & hope that fixes it.
                // over time, this will discard any segment that isn't a packet type number
                i++;
            }

            List<string> packetStrings = new();
            for (; i < strPieces.Length - 1; i += 2)
            {
                string packetData = strPieces[i] + "|" + strPieces[i + 1];
                packetStrings.Add(packetData);
            }

            // Malformation protection: We put the latest packet, which may be incomplete,
            // back onto the remainder if our buffer was full = we're likely missing data
            if (data.Length == DATA_BUFFER_SIZE)
            {
                remainder = System.Text.Encoding.UTF8.GetBytes(packetStrings[^1]);
                packetStrings.RemoveAt(packetStrings.Count - 1);
            }
        ...
        public static Packet DeserializeJson(string data)
        {
            ...
            switch (packetType)
            {
                ...
                case 71:
                    ...
                    break;
                case 72:
                    packet = JsonUtility.FromJson<EquipmentSlotPacket>(json);
                    break;
                case 73:
                    packet = JsonUtility.FromJson<EquipRequestPacket>(json);
                    break;
                case 74:
                    packet = JsonUtility.FromJson<EquippableItemTypePacket>(json);
                    break;
                case 75:
                    packet = JsonUtility.FromJson<ConsumableItemTypePacket>(json);
                    break;
                default:
    ...
    {
        public override int PacketType => 74;

        public DictionarySerializationWrapper<EquipmentSlot, string> SlotCriteriums;
        public List<string> SimpleStatStrings = new();
        public List<string> ConditionalStatStrings = new();
        public EquipmentType EquipmentType;
    }

    public class ConsumableItemTypePacket : ItemTypePacket
    {
        public override int PacketType => 75;

        public string UsageCriteriumsList;
    }
    ...
    public class EquipmentSlotPacket : Packet
    {
        public override int PacketType => 72;
    ...
    public class EquipRequestPacket : Packet
    {
        public override int PacketType => 73;
    ...
    public class ItemUseRequestPacket : Packet
    {
        public override int PacketType => 76;

        public long ItemTypeId;
        public int ItemUseParam1;
    }
